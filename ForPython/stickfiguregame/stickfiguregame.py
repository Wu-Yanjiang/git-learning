...
def within_y(co1,co2):
    if (co1.y1>co2.y1 and co1.y1<co2.y2) \
        or (co1.y>co2.y1 and co1.y2<co2.y2) \
        or (co2.y1>co1.y1 and co2.y1<co1.y2) \
        or (co2.y2>co1.y and co2.y2<co1.y2):
        return True
    else:
        return False
...
class StickFigureSprite(Sprite):
    ...
    def animate(self):
        ...
        if self.x<0:
            if self.y!=0:
                self.game.canvas_itemconfig(self.image,image=self.image_left[2])
            else:
                self.game.canvas_itemconfig(self.image,image=self.image_left[self.current_image])
        elif self.x>0:
            if self.y!=0:
                self.game.canvas_itemconfig(self.image,image=self.image_right[2])
            else:
                self.game.canvas_itemconfig(self.image,image=self.image_right[self.current_image])
